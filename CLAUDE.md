...
- `tsdoc.ts` — JSDoc/TSDoc parsing: `parseComment`, `applyToDeclaration`, `cleanComment`. `parseComment` filters `@module` JSDoc blocks per-block (a module comment attaches to the file's first statement in the AST; without the filter it would read as that statement's own docs — `extractModuleComment` owns module comments)
