...
- Legacy `export let` components — still-legal Svelte 5 syntax, but with no `$props()` declaration there's no anchor: zero props, no diagnostic, and the first documented `export let`'s JSDoc leaks into `docComment` (the doc walk's `$props()` terminator never fires)
