...
- `optional` — reflects a `?` token on the member (interface property/method signatures, type-alias properties from intersections/object literals, class property declarations). N/A for top-level declarations, constructors, or call/index signatures. The checker widens an optional property or parameter to include `undefined`; the checker-backed paths (component props, type-alias properties, `ParameterJson.type`) strip that back off so `optional: true` carries it alone, keeping any `null` member (`a?: string | null` → `"string | null"`, `a?: null` → `"null"`). Stripping is top-level only — a callable's `typeSignature` comes from `checker.signatureToString`, which has no flag to omit the widening and so renders `"(a: string, b?: number | undefined): boolean"`, and an optional property nested in a printed object type keeps it too (`b: {c?: boolean}` → `"{ c?: boolean | undefined; }"`). Treat `typeSignature` as the checker's canonical rendering (what an editor shows on hover) and the structured fields as normalized. AST-backed members (interface and class properties, read from the written annotation) preserve a written `| undefined` verbatim. Under `exactOptionalPropertyTypes` the checker doesn't widen at all, and an explicitly written `a?: string | undefined` — a distinct type from `a?: string` in that mode — is stripped to `"string"` all the same; distinguishing them needs the compiler flag, which extraction doesn't currently read
